...
export abstract class NightworkersContractsFactory extends NightworkersContracts {
  ...
  protected _useExposedContractVariations: boolean
    ...
    private async _init () {
      ...
      this.nightworkersGame.builder = await ethers.getContractFactory(debugPrefix + 'NightworkersGame')
      this.redLightDistrict.builder = await ethers.getContractFactory(debugPrefix + 'RedLightDistrict')
      this.candyMachine.builder = await ethers.getContractFactory(debugPrefix + 'CandyMachine')
      this.lottery.builder = await ethers.getContractFactory(debugPrefix + 'Lottery')
      this.tableGames.builder = await ethers.getContractFactory(debugPrefix + 'TableGames')
      this.backroom.builder = await ethers.getContractFactory(debugPrefix + 'Backroom')
    }
