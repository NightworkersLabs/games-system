...
const _readDirNightworkers = (basePath: string): ReadNFTAsset[] => {
...
const _readTraitsTypes = (nftFolder: ReadNFTAsset): ReadNFTAsset[] => {
...
const _getIndexFromDirName = (dirname: string) => {
  return Number(dirname.substring(0, dirname.indexOf("_"))) - 1;
};

const _readAssets = (traitFolder: ReadNFTAsset): ReadNFTAsset[] => {
...
export const _readTraitsTypesFlat = (
  directoryToRead: string,
): ReadNFTAsset[] => {
  return _readDirNightworkers(directoryToRead).flatMap(_readTraitsTypes);
};

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const produceNamesArrayFrom = (traitsPacks: TraitsPack[]): any[] => {
  //
  const output = [[], []];

  //
  for (const pack of traitsPacks) {
    output[pack.isHooker ? 0 : 1].push(pack.traits.map((e) => e.assetName));
  }
...
export const readNFTAssetsRepositoryFlat = (
  directoryToRead: string,
): ReadNFTAsset[] => {
  return _readTraitsTypesFlat(directoryToRead).flatMap(_readAssets);
};

export const readNFTAssetsRepositoryPacker = (
  directoryToRead: string,
): TraitsPack[] => {
  return _readTraitsTypesFlat(directoryToRead).map<TraitsPack>((e) => {
    //
    ...
    return {
      ...
      assetsIndices: assets.map((e) => e.assetIndex).sort((a, b) => a - b),
      traits: assets
        .map((e) => {
          return {
