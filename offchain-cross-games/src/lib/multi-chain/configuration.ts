...
export class BlockchainsRuntimes {
  ...
  //
  static async gather(
    env: IScraperEnvTemplate,
    configs: CasinoBlockchainConfiguration[],
  ): Promise<BlockchainsRuntimes> {
    //
    const gatheredRuntimes = await Promise.allSettled(
      configs.map((conf) =>
        BlockchainsRuntimes._checkAndGenerateRuntime(env, conf).catch((e) => {
          throw new Error(
            `Skipping BC n°${conf.chainId} runtime generation - ${e}`,
          );
        }),
      ),
    );
    ...
    if (okRuntimes.length === 0) {
      throw new Error(
        "No blockchain runtime could have been initialized... Aborting.",
      );
    }
  ...
  //
  private static async _checkAndGenerateRuntime(
    env: IScraperEnvTemplate,
    config: CasinoBlockchainConfiguration,
  ): Promise<CasinoBlockchainRuntime> {
    //
    ...
    if ((await str).toLowerCase() !== controller.address.toLowerCase()) {
      throw new Error(
        `[${controller.address}] is expected to be a validator of contract [${contract.address}] on blockchain ID (${config.chainId})`,
      );
    }
  ...
  public safeGetRuntime(chainId: number): CasinoBlockchainRuntime {
    ...
    if (runtime == null) {
      throw new Error(
        "Could not find runtime configuration for chainId " + chainId,
      );
    }
