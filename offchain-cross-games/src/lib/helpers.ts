...
//
export const getGasCostOfTx = (rcpt: ContractReceipt) => {
...
//
const _getGasCostOfTx = ({ gasUsed, effectiveGasPrice }: ContractReceipt) => {
  //
  ...
  //
  const gasPrice = parseFloat(formatUnits(effectiveGasPrice, "gwei")).toFixed(
    2,
  );
