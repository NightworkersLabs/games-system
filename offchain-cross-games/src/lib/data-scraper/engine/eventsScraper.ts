...
import { EventsScraperConfiguration } from './eventsScraperConfiguration'
...
//
export abstract class EventsScraper<T extends { blockSync: number, blockCreated: number }> extends BlockPoller {
  //
  ...
  constructor (
    ...
    contractName: string,
    client: PrismaClient,
    getEvtConfigurations: (contract: Contract, chainId: number) => EventConfigurations
  ) {
    //
    ...
    //
    this._evtConfigurations = getEvtConfigurations(this._contract, bcRuntime.chainId)
  ...
  override async _run () {
    ...
    const currentBCBlockNumber$ = this._produceBlockNumberPoller(this._contract).pipe(
      share()
    )
    ...
    return from(runtimes).pipe(
      mergeMap(r =>
        this._scanRuntime(currentBCBlockNumber$, r)
      )
    )
  }

  //
  private _scanRuntime (currentBCBlockNumber$: Observable<number>, runtime: EventRuntime<T>) {
    //
    return currentBCBlockNumber$.pipe(
      ...
      exhaustMap(currentBlockNumber =>
        ...
        this._mayBatchSyncEvents(runtime, currentBlockNumber).pipe(
          ...
          tap(r => this.__logR(r, ` Sleep for ${this._EVTS_POLL_WAIT_MS} ms`, 'discrete', true)),
          // then, wait until a trigger from...
          delayWhen(() =>
            race(
              // ... waiting configured amount of seconds
              interval(this._EVTS_POLL_WAIT_MS).pipe(
                tap(() => this.__logR(runtime, ' Waking, ready to sync on next block !', 'discrete', true))
              ),
              // ... the stop event
      ...
      finalize(() => { this.__logR(runtime, ' Exited.', 'discrete') })
    )
  }

  //
  private _mayBatchSyncEvents (runtime: EventRuntime<T>, currentBlockNumber: number): Observable<EventRuntime<T>> {
    ...
    return from(packs).pipe(
      ...
      concatMap(searchRange => {
        //
        this.__logRSR(runtime, searchRange, 'fetching events...', 'discrete', true)

        // fetch events and duplicate to db
        return from(runtime.evConfig.fetchDuplicate(searchRange[0], searchRange[1])).pipe(
          ...
          tap(howManyInserted => {
            ...
            //
            this.__logRSR(runtime, searchRange, `inserted ${howManyInserted} new events`)
          }),
          // update block of sync context to latest
          concatMap(() => this._updateSyncBlock(searchRange[1], runtime)),
          // returns runtime
          tap(() => this.__logR(runtime, `Synced to block ${searchRange[1]}`, 'discrete', true)),
  ...
  //
  private __logR (evRuntime: EventRuntime<T>, msg: string, color?: LogType, skippedInProd?: boolean) {
    const fMsg = `[${evRuntime.evConfig.eventName}]${msg}`
    this.__logB(fMsg, color, skippedInProd)
  }

  //
  private __logRSR (evRuntime: EventRuntime<T>, searchRange: [number, number], msg: string, color?: LogType, skippedInProd?: boolean) {
  ...
  private async _setupInitialEventsRuntimes (): Promise<EventRuntime<T>[]> {
    ...
    return Promise.all(
      this._evtConfigurations.map(async config => {
        //
        const syncContext = await this._upsertSyncContext(contractCreationBlock, config.eventName)
  ...
  /**
       ...
       */
  private async _upsertSyncContext (ccb: BigNumber, eventName: string): Promise<T> {
    //
    ...
    if (syncData) {
      ...
      if (dbBlock !== bcBlock) {
        throw new Error(`Contract creation block missmatch (DB: ${dbBlock}| BC: ${bcBlock}) for "${eventName}" event.`)
      }
      ...
      // logging
      this.__logB(` Created syncing context for "${eventName}" event, synced at [${initial.blockCreated}] block`)
  ...
  //
  protected abstract _createSyncData(creationBlock: BigNumber, eventName: string): Promise<T>
   ...
   */
  protected abstract _updateSyncBlock(blockSync: number, runtime: EventRuntime<T>): Promise<EventRuntime<T>>
}
