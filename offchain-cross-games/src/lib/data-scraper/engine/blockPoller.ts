...
import {
  BasicScraperEngine,
  type LogType,
} from "#/src/lib/data-scraper/engine/basicScraperEngine";
      ...
      expand((previousBlockNumber) =>
        of(null).pipe(
          ...
          delayWhen(() => {
            ...
            this.__logBCB(
              `Sleep for ${BlockPoller._BLOCK_NUM_POLL_WAIT_MS} ms`,
              "discrete",
              true,
            );
            ...
            return race([
              ...
              interval(BlockPoller._BLOCK_NUM_POLL_WAIT_MS).pipe(
                tap(() =>
                  this.__logBCB(
                    `Waited ${BlockPoller._BLOCK_NUM_POLL_WAIT_MS} ms, syncing...`,
                    "discrete",
                    true,
                  ),
                ),
          ...
          tap((fetchedBn) => {
            ...
            if (
              fetchedBn >
              (previousBlockNumber as number) /** COULD NOT BE UNDEFINED HERE */
            ) {
