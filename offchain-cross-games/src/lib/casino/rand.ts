...
export type BetExecutor<T> = (betTarget: T, howMuchChipsBet: number, fcr: FairlyComputedRandom) => Promise<RandomOutcome<T>>
type FlipCoinBetExecutor = BetExecutor<CoinFlipOutcome>
type RouletteBetExecutor = BetExecutor<RouletteOutcome>

//
export const coinFlipper: FlipCoinBetExecutor = async (betTarget, howMuchChipsBet, fcr) => {
  //
...
//
export const rouletteSpinner: RouletteBetExecutor = async (betTarget, howMuchChipsBet, fcr) => {
  //
  const rawOutcome = BigNumber.from(fcr.randomNumberProduced).mod(15).toNumber()

  //
  const outcome: RouletteOutcome = rawOutcome === 0
    ? 'Green'
    : (
      rawOutcome === 10 ||
        rawOutcome === 9 ||
  ...
  //
  const howMuchChipsWon = outcome !== betTarget
    ? 0
    : (
