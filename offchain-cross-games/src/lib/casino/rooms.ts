...
import type { BetRegisterer } from "#/src/lib/casino/db";
import { CasinoLockable } from "#/src/lib/casino/lockable";
import {
  type BetExecutor,
  type CoinFlipOutcome,
  type FairlyComputedRandom,
  type RandomOutcome,
...
export class Casino extends CasinoLockable {
  ...
  /**  */
  private _hasPlayedEnoughtForWithdraw(
    balance: ChipsBalance & { playsWithoutWD: number },
  ): number | true {
    // if already played enough, can withdraw
    if (balance.playsWithoutWD >= this._minimumPlaysBeforeWithdraw) return true;

    //
    const remainingGamesToPlay =
      this._minimumPlaysBeforeWithdraw - balance.playsWithoutWD;
    ...
    if (howManyGamesMaxPlayable <= remainingGamesToPlay)
      return howManyGamesMaxPlayable;
  ...
  public async convertChips(
    ...
    // access DB values...
    return this._accessDatabase(
      playerWalletID,
      chainId,
      async (client, balance, { contract }) => {
        // checks that owns more that 0 chips
        if (balance == null || balance.withdrawable <= 0) {
          throw new UserError(
            "no withdrawable chips, make sure that your balance is up to date",
          );
        }
        ...
        if (typeof hasPlayedEnough === "number") {
          throw new UserError(
            `you need to play x${hasPlayedEnough} more before being able to withdraw`,
          );
        }
        ...
        if ((await contract.isWithdrawPossible(amount)) === false) {
          throw new UserError(
            "bank do no have the funds to give you what is owed, please retry later",
          );
        }

        // ask for chips to be converted then user balance updated accordingly
        const tx = (await contract.convertChips(
          playerWalletID.toHexString(),
          amount,
        ...
        console.log(
          this.__getLog(
            playerWalletID,
            chainId,
            `${amount} $CHIPS converted back to owner, ${getGasCostOfTx(rcp)}`,
          ),
        );
  ...
  //
  public async interrogateBalance(
    playerWalletID: BigNumber,
    chainId: number,
  ): Promise<ChipsBalance> {
    // access DB values...
    return this._accessDatabase<ChipsBalance>(
      playerWalletID,
      chainId,
      async (client, balance, { contract }) => {
        // get total number of chips bought / airdropped by the account
        const { bought, airdropped } = (await contract.account(
          playerWalletID.toHexString(),
        )) as BalanceAccountability;
        ...
        if (balance === null) {
          ...
          console.log(
            this.__getLog(
              playerWalletID,
              chainId,
              `Initial ${this.__blDsp(newBalance)} $CHIPS balance initialized`,
            ),
          );
          ...
          console.log(
            chalk.yellow(
              this.__getLog(
                playerWalletID,
                chainId,
                `Unexpected $CHIPS balance state (DB: ${this.__blDspBase(
                  balance.tBought,
                  balance.tAirdropped,
                )} <> BC: ${this.__blDspBase(
                  bought,
  ...
  public async playWithChips<T = RouletteOutcome | CoinFlipOutcome>(
    ...
    // access DB values...
    return this._accessDatabase<BetResult<T>>(
      bet.playerWalletID,
      bet.chainId,
      async (client, balance) => {
        //
  ...
  //
  private _getRandomFromPayload(
    payload: TrustfulOrderPayloadWithNonce,
  ): FairlyComputedRandom {
    //
    let secretAsHexString = this._secretsStorage.tryExtractSecretFromHash(
      payload.hashedSecret,
    );
