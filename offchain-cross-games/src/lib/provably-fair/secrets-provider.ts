...
import { RandomGenerator } from './compliance'
...
export class SecretsStorage {
  ...
  constructor (secsBeforeAutoDispose: number = SecretsStorage.DEFAULT_SECONDS_BEFORE_AUTODISPOSE) {
    ...
    //
    if (mustLog) { console.log('[SecretsStorage] Ready.') }
  }
  ...
  shutdown () {
    //
    if (mustLog) { console.log('[SecretsStorage] shutting down...') }
  ...
  public requestSecretH (): string {
    ...
    //
    if (mustLog) { console.log(`[SecretsStorage] secret requested (hash : ${secret})`) }
  ...
  tryExtractSecretFromHash (hashedSecret: BigNumber): string | undefined {
    ...
    if (typeof mbResult !== 'undefined') {
      ...
      // may log on success
      if (mustLog) { console.log(`[SecretsStorage] secret extracted (hash : ${hashedAsHexStr})`) }
