...
import {
  DataEnvGenerator,
  isBearerAuthBypassAllowed,
} from "#env/data/generator";
...
const main = async () => {
  ...
  if (bearerAuthBypassAllowed) {
    console.log(
      chalk.green(
        "You can use the env-configured Bearer Token to bypass CORS-only API access :)",
      ),
    );
  }
  ...
  //
  webServer.listen(
    { port: defaultDataProviderServicePort, host: "0.0.0.0" },
    (err, address) => {
      //
      if (err) {
        console.error(err);
