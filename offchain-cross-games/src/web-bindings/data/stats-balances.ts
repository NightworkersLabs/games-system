...
import { getNetworkFilter } from './_'
...
//
export function bindBalancesStatsToWebServer (webServer: FastifyInstance, client: PrismaClient) {
  //
  async function getChipsBalanceAt (query: FromSchema<typeof cheapable>, pointInTime: Date) {
    //
  ...
  //
  async function getBalanceMovements (query: FromSchema<typeof cheapable>, after: Date) {
    //
    ...
    //
    const history = cb_after.reduce((out, i) => {
      //
  ...
  webServer.get<{ Querystring: FromSchema<typeof cheapable> }>(
    ...
    ({ query }) => client.chipsBalance.groupBy({
      by: ['chainId'],
      _sum: {
        tBought: true,
        tWithdrawed: true,
  ...
  webServer.get<{ Querystring: FromSchema<typeof cheapable> }>(
    ...
    async ({ query }) => {
      ...
      //
      const layout = uniqueFromChainIds<[time: number, balance: number]>(balance_before, balance_movements)
      ...
      entries.forEach(([chainId, array]) => {
        ...
        // sort by date asc
        balance_movements[chainIdN]?.sort((a, b) => a[0].getTime() - b[0].getTime())
