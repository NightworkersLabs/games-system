...
import { type CoinFlipOutcome, CoinFlipOutcomeV, coinFlipper, type RouletteOutcome, RouletteOutcomeV, rouletteSpinner } from '#casino/rand'
import { coinflipRegisterer, rouletteRegisterer } from '#casino/db'
import { UserError } from '#lib/helpers'
import { produceTOPWN, type TrustfulOrderPayloadWithNonce } from '#provably-fair/compliance'
...
function validateIdentity<T> (
  request: FastifyRequest,
  reply: FastifyReply,
  /** payload that contains the authentication data */
  payload: FromSchema<typeof identifyProof>,
  ...
  new Promise((resolve, reject) => {
    //
    const verification = ethers.utils.verifyMessage(IDText, payload.accSign).toLowerCase()
    if (verification !== payload.account) {
      throw new UserError('Cannot validate identity of requester. Please log-in again.')
    }
...
//
function produceTOPWNFromSchema ({ clientSeed, hashedSecret, expectedNonce }: FromSchema<typeof trustfulBet>): TrustfulOrderPayloadWithNonce {
...
const IDText = 'To Access Night Workers\'s Casino, you need to sign this text to authenticate yourself. This operation is absolutely free.'

//
export function bindCasinoToWebServer (webServer: FastifyInstance, casino: Casino) {
  //
  ...
  webServer.post<{ Body: FromSchema<typeof rouletteBet> }>(
    ...
    (req, rep) => {
      validateIdentity(req, rep,
        req.body,
        signedInAccount => casino.playWithChips(
          rouletteSpinner,
          rouletteRegisterer,
          {
