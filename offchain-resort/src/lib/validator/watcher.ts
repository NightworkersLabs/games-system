...
import { type RunContext, TrustedValidatorBound } from '#src/lib/tv-daemon'
...
import { BigNumber, Contract, type ContractInterface, type ContractReceipt, type ContractTransaction, type Event, type EventFilter } from 'ethers'
import type { Result } from 'ethers/lib/utils.js'
import { concat, concatAll, concatMap, filter, finalize, from, fromEvent as rxFromEvent, map, merge, Observable, takeWhile, tap } from 'rxjs'
...
export abstract class TrustedValidatorWatcher {
  ...
  protected abstract _orderExecutor(clientOrder: Result, responsePayload: TrustfulResponsePayload): Promise<ContractTransaction>
  ...
  //
  protected async _executeOrder (clientOrder: Result, responsePayload: TrustfulResponsePayload): Promise<ContractReceipt> {
  ...
  //
  constructor (controller: TrustedValidatorBound, secretsStorage: SecretsStorage, contractAddress: string, abi: ContractInterface, bootBlockNumber: number) {
    //
    this._contract = new Contract(contractAddress, abi, controller.trustedValidator)
  ...
  protected _logOrderInjection (clientOrder: Result) {
    ...
    console.log(
      chalk.grey(
        `${this.__logOrder(clientOrder)} event injected`
      )
    )
  }
  ...
  protected _logOrderInterception (clientOrder: Result) {
    ...
    console.log(
      chalk.grey(
        `${this.__logOrder(clientOrder)} event intercepted`
      )
    )
  }

  protected _mayLogOrderPFFailure (clientOrder: Result, mbSecret: string | undefined) {
    //
    if (!TVLogger.mustLog) return
  ...
  protected _logOrderRecovering () {
    ...
    console.log(
      chalk.grey(
        `${this.__logOrderE()} recovering events...`
      )
    )
  }
  ...
  protected _logOrderSkipping (clientOrder: Result) {
    ...
    //
    console.log(chalk.grey(`${this.__logOrder(clientOrder)} replayed, skipped.`))
  }

  protected _logOrderHandled (clientOrder: Result, receipt: TransactionReceipt) {
    //
    if (!TVLogger.mustLog) return
    console.log(`${this.__logOrder(clientOrder)} handled, ${getGasCostOfTx(receipt)}`)
  }
  ...
  protected _mayLogEventAsNotPFSafe (payload: TrustfulOrderPayloadWithNonce) {
    ...
    //
    const isCompliant = !payload.clientSeed.isZero() && !payload.hashedSecret.isZero()
  ...
  protected _logMissingNonceInDB (handledOrderNonce: number) {
    ...
    //
    console.log(chalk.yellow(`${this.__logOrderN(handledOrderNonce)} could not find recovered event in its database`))
  }
  ...
  protected _logError (err: string, clientOrder: Result) {
    ...
    console.log(
      chalk.red(
        `${this.__logOrder(clientOrder)} ${err}, skipped.`
      )
    )
  }
  ...
  //
  private async _generateTrustfulResponse (qto: QueriedTrustfulOrder): Promise<TrustfulResponsePayload> {
    ...
    if (event.hashedSecret.isZero()) {
      ...
      const mbRevealed = this._secretsStorage.tryExtractSecretFromHash(event.hashedSecret)
 
      //
      this._mayLogOrderPFFailure(qto.order, mbRevealed)

      // check if revelation succeeded
      usedSecret = typeof mbRevealed !== 'undefined'
   ...
   */
  private async _getTrustfulOrderEventOf (qto: QueriedTrustfulOrder): Promise<TrustfulOrderPayloadWithNonce> {
    //
    ...
    //
    const events = await this._contract.queryFilter(filter, qto.blockNumber, qto.blockNumber)
    const [first] = events

    //
    if (events.length !== 1 || first == undefined) {
      throw new Error(`[${this._REQUEST_EV} : ${qto.nonce}] not finding associated Trustful Order (${events.length} found)`)
    }
  ...
  private async _getEventsFromFilter (eventFilter: EventFilter | undefined, contractCreationBlockNumber: number): Promise<QueriedTrustfulOrder[]> {
    if (eventFilter == undefined) {
      throw new Error('Event filter is not defined')
    }
    
    // pack them by 2048 (fuji events limit per request)
    const evRanges = packUnoverlapped(contractCreationBlockNumber, this._bootBlockNumber, 2048)

    //
    const orders: QueriedTrustfulOrder[] = []
    for (const [from, to] of evRanges) {
      orders.push(
        ...await this._contract.queryFilter(eventFilter, from, to)
  ...
  private async _recoverEventsToProcess (): Promise<QueriedTrustfulOrder[]> {
    ...
    //
    const contractCreationBlock = (await this._contract.CREATION_BLOCK() as BigNumber).toNumber()
    ...
    this._latestProcessedId = processedIds.length === 0 ? -1 : Math.max(...processedIds)

    // get all the orders IDs which have not been processed
    this._unprocessedOrderIds = ordered.filter(x => !processedIds.includes(x.nonce)).map(o => o.nonce)

    // only forward orders that have NOT been processed already
    const recovered = ordered.filter(ev => this._unprocessedOrderIds.includes(ev.nonce))
  ...
  private _isOrderProcessingAllowed (orderNonce: number): OrderProcessAllowance | boolean {
    if (this._unprocessedOrderIds.includes(orderNonce)) return OrderProcessAllowance.AllowedRecovered
    if (orderNonce > this._latestProcessedId) return OrderProcessAllowance.AllowedNew
    return false
  }

  //
  private _updateReplayDatabase (allowanceType: OrderProcessAllowance, handledOrderNonce: number) {
    // if recovering...
    if (allowanceType === OrderProcessAllowance.AllowedRecovered) {
      ...
      const indexToDelete = this._unprocessedOrderIds.indexOf(handledOrderNonce)
  ...
  private _getRecoverEventsObservable (): Observable<QueriedTrustfulOrder> {
    return from(this._recoverEventsToProcess())
      .pipe(
        concatAll()
      )
  }
  ...
  private _getInjectedEventsObservable (): Observable<QueriedTrustfulOrder> {
    return fromEvent(this._injectedEe, 'injected')
      .pipe(
  ...
  private _getBlockchainEventsObservable (): Observable<QueriedTrustfulOrder> {
    return fromEvent<Event[]>(this._contract, this._REQUEST_EV)
      .pipe(
  ...
  public tryInject (receipt: ContractReceipt): boolean {
    const matching = receipt.events?.filter(e => e.event === this._REQUEST_EV) ?? []
  ...
  public run (rc: RunContext) {
    ...
    const c = concat(
      ...
      merge(
        ...
        rc.stopper$.pipe(
          map(() => null as unknown as QueriedTrustfulOrder)
        )
      )
    ).pipe(
