...
import { lastValueFrom, fromEvent as rxFromEvent, merge, takeWhile, tap, share, Observable } from 'rxjs'
import { EventEmitter } from 'node:events'
import { BigNumber, type ContractReceipt, Signer, type Event } from 'ethers'

import { TrustedValidatorWatcher, TrustfulOrderPayload } from '#src/lib/validator/watcher'
...
export class TrustedValidatorDaemon extends TrustedValidatorBound {
  ...
  private _runContext: RunContext = {
    keepAlive: true,
    stopper$: fromEvent(this._evE, 'stop').pipe(
      share()
    )
  }

  //
  constructor (args: TVDaemonArgs, secretsStorage: SecretsStorage, recoverFromBlockNumber: number) {
    ...
    //
    this._instances = [
      new NWGWatcher(this, this._secretsStorage, args.nwContractAddress, NightworkersGameABI, recoverFromBlockNumber),
      new RLDWatcher(this, this._secretsStorage, args.rldContractAddress, RedLightDistrictABI, recoverFromBlockNumber),
      new CFWatcher(this, this._secretsStorage, args.tgContractAddress, TableGamesABI, recoverFromBlockNumber),
      new RouletteWatcher(this, this._secretsStorage, args.tgContractAddress, TableGamesABI, recoverFromBlockNumber),
      new LotteryWatcher(this, this._secretsStorage, args.lotteryContractAddress, LotteryABI, recoverFromBlockNumber)
    ]
  }
  ...
  stop () {
    //
    if (TVLogger.mustLog) { console.log('[TrustedValidator] Stopping ...') }
  ...
  async exec () {
    ...
    const jobs = merge(
      ...this._instances.map(i =>
        i.run(this._runContext)
      )
    )

    //
    if (TVLogger.mustLog) { console.log('[TrustedValidator] Ignited.') }
    ...
    //
    if (TVLogger.mustLog) { console.log('[TrustedValidator] Stopped.') }
  }
