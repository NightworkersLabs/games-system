...
async function main () {
  ...
  //
  const provider = new ethers.providers.JsonRpcProvider(env.RPC_URL, env.CHAIN_ID)
  const wallet = TrustedValidatorEnv.getWalletFromEnv(env.MNEMO_OR_PRIV_KEY, provider)
