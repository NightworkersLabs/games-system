import { BaseContract, Contract, ContractFactory, ContractTransaction } from 'ethers'
...
export class NightworkersContracts extends NightworkersContractsBase {
  ...
  private async _transferOwnership (expectedNewOwner: string, contractContexts: ContractContext[]) {
    ...
    for (const { contract, contractName } of contractContexts) {
      ...
      try {
        ...
        //
        const tx = await contract.getFunction("transferOwnership").send(expectedNewOwner)
        await tx.wait()
      //
      } catch (e) {
  ...
  async transferOwnershipTo (newOwnerAddress: string) {
    ...
    const balanceBeforeDeploy = await ethers.provider.getBalance(deployer.address)
    ...
    // calculate price
    const balanceAfterDeploy = await await ethers.provider.getBalance(deployer.address)
