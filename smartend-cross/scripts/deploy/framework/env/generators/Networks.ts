...
import { HISTORIZED_NETWORKS_IGNORED } from '../../NWContracts'
  ...
  HHConfig.networks == null
    ...
    : Object.fromEntries(
      ...
      // not in ignored networks
        .filter(([networkName]) => !HISTORIZED_NETWORKS_IGNORED.includes(networkName))
      ...
      // has rpc url and chain id
        .filter(([, config]) => config != null && 'chainId' in config && 'url' in config)
      ...
      // try to findout if this network is cheap or not
        .map(([networkName, config]) => ([
          networkName, {
            ...config,
            ...(checkCheapness(networkName, config) ? { cheap: true } : null)
          } as Required<ConfigurableNetworkBase>
        ] as const))
    )
...
const checkCheapness = (name: string, rpcConfig: ConfigurableNetworkBase) => {
  if (name.toLowerCase().includes('test') || name.toLowerCase().includes('oerli') || ['hardhat'].includes(name)) return true
  if (rpcConfig.url == null) return false

  const url = rpcConfig.url.toLowerCase()

  //
  return url.includes('test') || url.includes('localhost') || url.includes('127.0.0.1') || url.startsWith('http://')
}
