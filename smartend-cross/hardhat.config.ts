...
function _tryReadingDeployConfig () : HDAccountsUserConfig | undefined {
  const log = `[Hardhat] Deploy configuration informations from project root "${deployConfigFileName}"`
  try {
    const parsed = JSON.parse(fs.readFileSync(deployConfigFileName, 'utf-8')) as HDAccountsUserConfig
    console.log(`${log} found !`)
    return parsed
  } catch {
    console.log(`${log} could not be parsed. Deploying on mainnet networks will not be possible.`)
    return undefined
  }
}
...
const config: HardhatUserConfig = {
  ...
  solidity: {
    compilers: [
      {
        version: '0.8.15', /** >= 0.8.13 with IR removes "stack too deep" issues */
        settings: {
