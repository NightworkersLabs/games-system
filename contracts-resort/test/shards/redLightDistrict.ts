...
export const redLightDistrict = () =>
  describe("Red Light District", () => {
    before(async function () {
      ...
      const secsHookerEarnings =
        (await this.redLightDistrict.SEC_LOLLY_RATE()) as BigNumber;
    ...
    describe("Scenario n°1: Stake 1 token, and unstake ASAP (no preleminary gains staked)", () => {
      describe("Hooker", () => {
        ...
        step(
          "should NOT be able to stake with incorrect claim tax",
          async function () {
            // no tax
            await expect(
        ...
        step(
          "Warp 1 day, still should NOT be able to unstake",
          async function () {
        ...
        step(
          "should be able to unstake after 2 days (maybe with gains)",
          async function () {
            ...
            if (!amountWon.isZero()) {
              expect(amountWon).to.be.closeTo(
                parseEther("20000"),
                parseUnits("1", "gwei"),
              );
            }
      ...
      describe("Pimp", () => {
        ...
        step(
          ...
          async function () {
            // no tax
            await expect(
              this.redLightDistrict.unstakeMany([this.pimps[0]], [0, 0]),
            ).to.be.revertedWith("InvalidClaimTaxAmount");
        ...
        step(
          "should be able to claim instantly (maybe with gains of previously)",
          async function () {
            //
            const unaccounted =
              (await this.redLightDistrict.unaccountedRewards()) as BigNumber;
        ...
        step(
          "should be able to unstake instantly (without gains)",
          async function () {
            //
            const rcpt: ContractReceipt = await this.redLightDistrict
    ...
    describe("Scenario n°2: staking hookers, claiming gains, staking pimps, claiming gains", async function () {
      step(
        "should be able to stake hooker then claim gains 1 days after",
        async function () {
          //
          await this.redLightDistrict
            .putManyToWork([this.hookers[0]])
          ...
          //
          this.hookerEarned = rcpt.events.filter(
            (x) => x.event === "TokenClaimed",
          )[0].args.earned as BigNumber;
          const expectedWinnings = (this.dailyEarnings as BigNumber)
            .mul(80)
            .div(100);
          expect(this.hookerEarned).to.be.closeTo(
            expectedWinnings,
            parseEther("1"),
          );
      ...
      step(
        "should be able to stake pimp, then claim all that was waiting in the pot",
        async function () {
          //
          await this.redLightDistrict
            .putManyToWork([this.pimps[0]])
          ...
          //
          const balanceBefore = await this.lolly.balanceOf(
          ...
          //
          const balanceAfter = await this.lolly.balanceOf(
            this.users.owner.address,
          );

          //
          const pimpWon = (balanceAfter as BigNumber).sub(
            balanceBefore as BigNumber,
          );
      ...
      step(
        "should be able to claim gains-less, then unstake gains-less",
        async function () {
          //
          const balanceBefore = await this.lolly.balanceOf(
            this.users.owner.address,
          ...
          //
          const balanceAfter2 = await this.lolly.balanceOf(
            this.users.owner.address,
          );
    ...
    describe("Scenario n°3: moving pimps", async function () {
      ...
      step(
        "should be able to claim both after 1 day, for full rewards",
        async function () {
          //
          await mineTimeWarpSecs(this.oneDay);
      ...
      step(
        "wait 1 day, claim hooker, stake +1x pimp : the older should get the reward, not the newer",
        async function () {
      ...
      step(
        "wait 1 day, claim hooker + both pimps, both pimps should gain the same thing",
        async function () {
          //
          await mineTimeWarpSecs(this.oneDay);
          ...
          const rcpt: ContractReceipt = await this.redLightDistrict
            .claimMany(
              [this.pimps[0], this.pimps[1], this.hookers[1]],
              [0, 0],
              {
      ...
      step(
        "wait 2x1 day, claim hooker each day, claim pimp 1 day 1, claim both pimps day 2: pimp 2 should gain 2 days of rewards",
        async function () {
          //
          await mineTimeWarpSecs(this.oneDay);
...
// HELPER
const expectEarnedFromEvts = (
  events: Event[],
  config: { tokenId: number; exepectedEther: string | BigNumber }[],
) => {
  config.forEach(({ tokenId, exepectedEther }) => {
    //
    const ev = events.find(
      (x) => x.args.tokenId === tokenId && x.event === "TokenClaimed",
    );
    ...
    expect(ev.args.earned).to.be.closeTo(
      typeof exepectedEther === "string"
        ? parseEther(exepectedEther)
        : exepectedEther,
      parseUnits("1", "gwei"),
    );
  });
