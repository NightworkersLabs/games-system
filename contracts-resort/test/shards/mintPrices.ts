    ...
    describe("Gen 1+", () => {
      it("should have no $LOLLY cost until Gen1 (first 10k)", async function () {
        const payableMintPrice =
          await this.nightworkersGame.lollyMintCostOf(10_000);
      ...
      it("should have 20k $LOLLY cost below 40% of pool (< 20k)", async function () {
        ...
        expect(await this.nightworkersGame.lollyMintCostOf(10_001)).to.eq(
          expected,
        );
        expect(await this.nightworkersGame.lollyMintCostOf(20_000)).to.eq(
          expected,
        );
      });
      ...
      it("should have 40k $LOLLY cost below 80% of pool (<= 40k)", async function () {
        ...
        expect(await this.nightworkersGame.lollyMintCostOf(20_001)).to.eq(
          expected,
        );
        expect(await this.nightworkersGame.lollyMintCostOf(40_000)).to.eq(
          expected,
        );
      });
      ...
      it("should have 80k $LOLLY cost above 80% of pool (> 40k)", async function () {
        ...
        expect(await this.nightworkersGame.lollyMintCostOf(40_001)).to.eq(
          expected,
        );
        expect(await this.nightworkersGame.lollyMintCostOf(50_000)).to.eq(
          expected,
        );
      });
      ...
      it("should NOT get a price for a token over 50k limit", async function () {
        await expect(
          this.nightworkersGame.lollyMintCostOf(50_001),
        ).to.be.revertedWith("token out of bonds");
      });
    });
  });
