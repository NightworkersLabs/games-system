...
export const backroom = () =>
  describe.only("Backroom (LOLLY > AVAX yield aggregator)", () => {
    before(async function () {
      //
      this.mintLOLLYAndDeposit = async (
        staker: SignerWithAddress,
        howMuch: BigNumber,
      ) => {
        // give LOLLY to owner
        await this.lolly.$_mint(staker.address, howMuch).then((e) => e.wait());

        // As Owner, increaseAllowance full deposit
        await this.lolly
          .connect(staker)
          .increaseAllowance(this.backroom.address, howMuch)
          .then((e) => e.wait());
    ...
    describe("Specific interactions", () => {
      before(async function () {
        // give LOLLY to owner
        await this.lolly
          .$_mint(this.users.owner.address, parseEther("10000"))
          .then((e) => e.wait());

        //
        await this.lolly
          .connect(this.users.owner)
          .increaseAllowance(this.users.owner.address, parseEther("10000"))
          .then((e) => e.wait());
      });
      ...
      after(async function () {
        ...
        await this.lolly
          .$_burn(this.users.owner.address, parseEther("10000"))
          .then((e) => e.wait());

        //
        await this.lolly
          .connect(this.users.owner)
          .decreaseAllowance(this.users.owner.address, parseEther("10000"))
          .then((e) => e.wait());
      });
      ...
      it("should not be able to send LOLLY directly to backroom", async function () {
        ...
        //
        await expect(
          this.lolly.connect(staker).transfer(this.backroom.address, howMany),
    ...
    describe("Basic schedule", () => {
      step(
        "must be able to deposit with no round scheduled",
        async function () {
          //
      ...
      step("must be able to schedule a round", async function () {
        ...
        this.roundBlock = await (
          this.backroom.scheduleRound(parseEther("0.1"), 0, 60, {
            value: roundReward,
        ...
        //
        const balance = await this.backroom.provider.getBalance(
      ...
      step("should NOT be able to withdraw", async function () {
        await expect(this.backroom.mayClaimDoWithdraw(0)).to.be.revertedWith(
          "WithdrawForbidden",
        );
      });

      step(
        "should NOT be able to claim with more than staked",
        async function () {
          await expect(
      ...
      step(
        "prevent player who did not stake to claim any rewards",
        async function () {
          //
      ...
      step(
        "should NOT be able to claim with much more than staked",
        async function () {
          //
          const rcpt = await (
            this.backroom.mayClaimDoWithdraw(
              parseEther("20000"),
            ) as Promise<ContractTransaction>
          ).then((e) => e.wait());
