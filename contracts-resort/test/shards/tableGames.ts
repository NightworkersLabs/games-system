...
export const tableGames = () =>
  describe('Table Games', () => {
    ...
    step('should NOT be able to play without minimum funds', async function () {
      await expect(this.tableGames.flipCoin(true, [0, 0])).to.be.revertedWith('minimum rolling liquidities not met')
      await expect(this.tableGames.betOnGreen([0, 0])).to.be.revertedWith('minimum rolling liquidities not met')
      await expect(this.tableGames.betOnRed([0, 0])).to.be.revertedWith('minimum rolling liquidities not met')
      await expect(this.tableGames.betOnBlack([0, 0])).to.be.revertedWith('minimum rolling liquidities not met')
    })

    step('feeding rolling funds...', async function () {
      //
      const targetRolling = await this.tableGames.MINIMUM_ROLLING() as BigNumber

      //
      await this.users.owner.sendTransaction({
        to: this.tableGames.address,
        value: targetRolling
      }).then(e => e.wait())

      //
      const rolling = await this.tableGames.provider.getBalance(this.tableGames.address)
    ...
    step('should NOT be able to play with < than minimum bet', async function () {
    ...
    step('should NOT be able to play with > than maximum bet', async function () {
