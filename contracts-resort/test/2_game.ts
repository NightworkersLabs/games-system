...
import { rewardEstimation } from './shards/rewardEstimation'
import { tokenMetadata } from './shards/tokenMetadata'
import { mintPrices } from './shards/mintPrices'
import { tableGames } from './shards/tableGames'
import { redLightDistrict } from './shards/redLightDistrict'
import { royalties } from './shards/royalties'
import { backroom } from './shards/backroom'
import { lottery } from './shards/lottery'

import TVDInstance from 'scripts/deploy/framework/TVDHelper'

import { MintOrderer } from 'test/_helpers'

import { TVLogger } from '@offchain-service/lib/params'
  ...
  describe('Game itself', () => {
    ...
    describe('Default state right after deployment', () => {
      ...
      it('should have correct initial payable token pool (10K)', async function () {
        const payableToken = await this.nightworkersGame.PAYABLE_TOKENS_UNTIL() as number
        expect(payableToken).to.equals(10_000)
      })

      it('should have correct $LOLLY base price (20k)', async function () {
        const payableToken = await this.nightworkersGame.MINT_BASE_PRICE() as BigNumber
      ...
      it('should give at which number of Gen 0 token a token starts to be scarce (9k - +20% price)', async function () {
        const scarceAt = await this.nightworkersGame.PAYABLE_TOKENS_SCARCE_AT() as number
        expect(scarceAt).to.equals(9000)
      })

      it('should NOT allow staking', async function () {
        const isAllowed = await this.redLightDistrict.isStakingAllowed() as boolean
      ...
      it('should have possible notoriety score for Pimp between appropriate values', async function () {
        const minPimpNotoriety = await this.nightworkersGame.MIN_PIMP_NOTORIETY_SCORE() as number
        const maxPimpNotoriety = await this.nightworkersGame.MAX_PIMP_NOTORIETY_SCORE() as number
      ...
      describe('Changing payable Price (Gen0, AVAX)', () => {
        ...
        it('price should have effectively changed', async function () {
          const basePrice = await this.nightworkersGame.getBasePayableMintPrice() as BigNumber
        ...
        it('should get correct whitelisted price (-20% on the newly-set price)', async function () {
          const payableMintPrice = await this.nightworkersGame.getWhitelistedPayableMintPrice()
        ...
        it('should get correct "scarce" price (+20% on the newly-set price)', async function () {
          const payableMintPrice = await this.nightworkersGame.getScarcePayableMintPrice()
    ...
    describe('When assets are imported', () => {
      before(async function () {
        ...
        this.getRandomUInt16 = () => randomBytes(2).reduce((a, b) => (a << 8) + b)
      })

      it('should not pick a trait index above uploaded assets limit (TraitPack x Assets)', async function () {
        //
        for (const { isHooker, traitTypeIndex, assetsIndices } of this.nwContext.nftAssets) {
          //
          const howManyAssets = assetsIndices.length

          //
          const tries = Array.from({ length: howManyAssets })
            .map(() =>
      ...
      describe('Whitelist...', () => {
        describe('Before being in whitelist mode', () => {
          describe('As Non WL', () => {
            it('should NOT be whitelisted by default', async function () {
              const wlLeft = await this.nightworkersGame.usableWhitelistTicketsLeft() as number
          ...
          describe('As WL', () => {
            ...
            step('should have correct initial buyable token count (5)', async function () {
              //
        ...
        describe('When being in whitelist mode', () => {
          ...
          describe('As Whitelisted', () => {
            ...
            step('whitelisted address should be able to mint 1 immediately', async function () {
              await this.mintOrderer.checked({ signer: this.users.wl, amount: 'wl' })
            })

            step('should be able to mint the 4 tokens remaining', async function () {
              await this.mintOrderer.checked({ signer: this.users.wl, amount: 'wl', howMany: 4 })
            })

            step('should NOT be able to mint after reaching the 5 tokens minting limit', async function () {
              await expect(
      ...
      describe('When game is in public mode', () => {
        before(async function () {
          ...
          await this.nightworkersGame.declarePublicLaunch().then(e => e.wait())
        })
        ...
        describe('As Broke', () => {
          it('should NOT allow to mint if no balance', async function () {
            ...
            await expect(
              this.mintOrderer.raw({ signer: this.users.broke })
            ).to.be.reverted
          })
        })
        ...
        describe('Pricing lifecycle', () => {
          ...
          step('should NOT be able to acheive multiple mints if the price goes from standard to scarce', async function () {
            await expect(
          ...
          step('should notice a pricing go up when reaching 9/10 of Gen 0 of +20%', async function () {
            expect(
          ...
          step('should NOT be able to acheive multiple mints if the price goes from AVAX to $LOLLY', async function () {
            await expect(
          ...
          step('should NOT pay in AVAX once Gen 0 is depleted', async function () {
            await expect(
              this.mintOrderer.raw()
            ).to.be.revertedWith('cannot spend AVAX on Gen 1')
          })
          ...
          step('should be able to mint 2 NFT with $LOLLY at 20k + 40k', async function () {
          ...
          step('should be able to mint 1 NFT with $LOLLY 40k', async function () {
          ...
          step('should be able to mint 1 NFT with $LOLLY 60k', async function () {
            await this.mintingLOLLY('60000')
            await this.mintOrderer.checked({ amount: 'free' })
            await this.checkEmptyLOLLYBalance()
          })
          ...
          step('should NOT be able to mint once limit is reached', async function () {
            await expect(
              this.mintOrderer.raw()
