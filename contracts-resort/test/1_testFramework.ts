...
export const testFramework = () =>
  describe("Test framework", () => {
    describe("Assets Packer", () => {
      const _testingContiguousTraitsTypes = (packs: TraitsPack[]) => {
        let i = 0;
        packs.forEach((e) => {
      ...
      const _testingContiguousAssetsIndices = (packs: TraitsPack[]) => {
        packs.forEach((p) => {
          let i = 0;
      ...
      describe("For Hookers", () => {
        before(async function () {
          this.hookerAssets = this.nwContext.nftAssets.filter(
            (e) => e.isHooker,
          );
        });
