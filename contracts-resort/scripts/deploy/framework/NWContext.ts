...
import { readNFTAssetsRepositoryPacker } from "@nightworkerslabs/nft-assets/utilities";
...
export class NightworkersContext extends NightworkersContractsFactory {
  ...
  protected async _deploy(params: IgniteParameters) {
    ...
    if (!this._useExposedContractVariations) {
      //
      if (
        params.taxesAndRevenueRecipientAddress == null ||
        params.taxesAndRevenueRecipientAddress.length === 0
      ) {
        throw new Error(
          "Not supplying the marketing multisig address in release mode is forbidden !",
        );
      }
      ...
      if (params.tokenURIBase == null || params.tokenURIBase.length === 0) {
        throw new Error(
          "Not supplying the tokenURI base of the associated dApp is forbidden in release mode !",
        );
      }
    } /* ... else, if in debug mode */ else {
      //
    ...
    if (params.ERC20TokenContractAddress == null) {
      ...
      try {
        ...
        throw new Error(
          "Could not find appropriate ERC20 on the network. Please check that the address is a valid contract with expected interface.",
        );
      }
    }

    //
    await this.candyMachine.deployW(params.tokenURIBase ?? "");
    logUpdate(getLog());

    //
    await this.backroom.deployW(this.lolly.contract.address);
    logUpdate(getLog());
    ...
    await this.redLightDistrict.contract
      .setMintingGame(this.nightworkersGame.contract.address)
      .then((e) => e.wait());
    await this.candyMachine.contract
      .setMintingGame(this.nightworkersGame.contract.address)
      .then((e) => e.wait());
    logUpdate(getLog());
    ...
    const mayAddAsERC20Controller = async (address: string) => {
      const isAlreadyController: boolean =
        await this.lolly.contract.isController(address);
    ...
    const mayInvalidateAsERC20Receiver = async () => {
      ...
      //
      const isBRAlreadyInvalidated: boolean =
        await lollyCtr.invalidReceiver(brAddr);
    ...
    if (params.publicLaunch) {
      await this.nightworkersGame.contract
        .declarePublicLaunch()
        .then((e) => e.wait());
    }
  ...
  async uploadAllAssets(deployWithEmptyData?: boolean) {
    ...
    let i = 0;
    const getLog = () =>
      `- Uploading assets... (${i} / ${this.nftAssets.length})`;
    logUpdate(getLog());
