...
export class NightworkersContracts extends NightworkersContractsBase {
  ...
  async massMint(
    ...
    let failedMint = 0;
    console.log(
      `Minting ${howManyToMint}, ${maxByIteration} by ${maxByIteration} !`,
    );
    ...
    const _log = (btl: boolean) => {
      ...
      process.stdout.write(
        `[${mustMint - howManyToMint} / ${mustMint}] (Failed : ${failedMint}) mints remaining...`,
      );
    };
    ...
    while (howManyToMint > 0) {
      //
      const howMuchToMint =
        maxByIteration > howManyToMint ? howManyToMint : maxByIteration;

      //
      await this.nightworkersGame.contract
        .mint(howMuchToMint, stake, {
          value: price.mul(howMuchToMint),
        })
        .then((e) => e.wait())
        .catch((_) => {
          failedMint += howMuchToMint;
        });
  ...
  async generateWhitelistedAccounts(howManyToGenerate: number) {
    ...
    if (howManyToGenerate < 0) {
      throw new Error("cannot generate whitelisted accounts");
    }
    ...
    const packBy = 100;
    const batches = this._splitWalletAddressesIntoBatches(
      generatedWallets,
      packBy,
    );
    ...
    for (const batch of batches) {
      await this.nightworkersGame.contract
        .grantManyWhitelistTickets(batch, 3)
        .then((e) => e.wait());
    }

    //
    console.log(
      `   > Confirmed ${howManyToGenerate} whitelisted addresses registered to contract`,
    );
  ...
  //
  private _splitWalletAddressesIntoBatches(
    wallets: Wallet[],
    packBy: number,
  ): Array<Array<string>> {
    const packsIndicies = Array.from(
      { length: Math.ceil(wallets.length / packBy) },
      (_e, i) => i,
    );
  ...
  private _generateWalletAccounts(howMany: number): Wallet[] {
    return Array.from({ length: howMany }).map(() =>
  ...
  private _createWalletsCSV(wallets: Wallet[]) {
    ...
    //
    console.log(
      `   > Generated ${wallets.length} wallet addresses into ${fileOutputPath}`,
    );
  }
}
