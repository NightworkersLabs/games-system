...
import type { ITrustedValidatorEnv } from "@nightworkerslabs/offchain-resort/src/env/ITrustedValidatorEnv";

import HHConfig from "#/hardhat.config";
import {
  getCurrentRPCUrl,
  IEnvGenerator,
} from "#/scripts/deploy/framework/env/generators/IEnvGenerator";
...
export class TrustedValidatorEnvGenerator extends IEnvGenerator<ITrustedValidatorEnv> {
  ...
  override generate(context: NightworkersContext) {
    return {
      ...
      MNEMO_OR_PRIV_KEY:
        TrustedValidatorEnvGenerator._getTrustedValidatorPrivateKeyOrMnemo(),
