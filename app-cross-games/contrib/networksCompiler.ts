...
import more from '#/config/networks.more'
...
/** Network specific options relative to dApp behavior */
export interface NetworkDAppOptions {
  /** consider bank validator to be broke (hence displaying a warning) when beeing bellow said value (as ETH, not WEI) */
  validatorBrokeBelow: string
  /** start to show bank balance when it is above configured (as ETH, not WEI) */
  showBankBalanceAbove: string
}
...
export interface BasicNetworkMetadata {
  ...
  logo: string
  /** */
  dappOptions?: NetworkDAppOptions
}
...
//
export interface WithHexChainId {
  chainIdHex: string
}
...
//
export type CompiledNetwork = NetworkDefinition & WithHexChainId & BaseNetwork & Partial<WithForksNetwork>
