...
import { Prisma, PrismaClient, type PrismaPromise } from '#prisma/client/index.js'
import { Contract, type EventFilter, type Event, BigNumber } from 'ethers'
import { formatUnits } from 'ethers/lib/utils.js'

//
export type EventsDatabaseFiller<T> = (eventsToInsert: T[]) => PrismaPromise<Prisma.BatchPayload>
...
export const decimalParser = (toParse: BigNumber | number | any) => {
  if (typeof toParse === 'number') {
    ...
    throw Error('Unhandled number-like received from blockchain => ' + String(toParse))
  }
}
...
export abstract class EventsScraperConfiguration<T> {
  ...
  protected readonly abstract inserter: EventsDatabaseFiller<T>
  protected readonly abstract formatter: EventsFormatter<T>

  protected readonly _client: PrismaClient
  protected readonly _contract: Contract
  protected readonly _chainId: number

  constructor (contract: Contract, client: PrismaClient, eventName: string, chainId: number) {
    //
  ...
  async fetchDuplicate (fromBlock: number, toBlock: number) {
    //
    const raw = await this._contract.queryFilter(this.eventFilter, fromBlock, toBlock)
    if (raw.length === 0) return 0

    // genenerate promise formatters and wait for them all to succeed
    const formatted = await Promise.all(
      raw.map(this.formatter)
    )
